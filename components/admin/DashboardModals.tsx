...
import React, { useState, useContext, ChangeEvent, useEffect, useRef, useMemo } from 'react';
import { AppContext } from '../../App';
import { type Order, type Product, type BankAccount, Category, type PromoCode, type ProductDiscount, type CustomOrderRequest, QrisInfo, WarrantyClaim, User, Courier, ColorOption, Bundle, BundleItem, SpecialCollection, SizeChart, SizeDetail, SizeMeasurement } from '../../types';
...
export type ModalState = 
    ...
    | { type: 'add-size-chart' } | { type: 'edit-size-chart'; chart: SizeChart } | { type: 'delete-size-chart'; chart: SizeChart };
...
const measurementFieldsConfig: Record<string, { key: keyof SizeMeasurement; label: string }[]> = {
    [Category.TSHIRT]: [
...
const AddEditSizeChartModal: React.FC<{ chart?: SizeChart; onSave: (data: SizeChart) => void; onClose: () => void }> = ({ chart, onSave, onClose }) => {
    const isEditing = !!chart;
    const isInitialMount = useRef(true);
    const [formData, setFormData] = useState<SizeChart>(
        chart || {
            id: '',
            name: '',
            category: Category.TSHIRT,
            style: '',
            details: [{ size: 'S', measurements: {} }],
        }
    );
    ...
    useEffect(() => {
        if (isInitialMount.current) {
            ...
            // When category changes, reset the style/subcategory
            setFormData(prev => ({ ...prev, style: '' }));
        }
    }, [formData.category]);

    useEffect(() => {
        if (formData.category && formData.style) {
            const newName = `${formData.category} - ${formData.style}`;
            setFormData(prev => ({ ...prev, name: newName }));
        }
    }, [formData.category, formData.style]);
    
    const subcategoryOptions = useMemo(() => {
        const selectedCat = categoryMenu.find(c => c.name === formData.category);
        return selectedCat?.subCategories || [];
    }, [formData.category]);

    const measurementFields = measurementFieldsConfig[formData.category] || [];
    ...
    const handleDetailChange = (index: number, field: keyof SizeDetail, value: any) => {
        const newDetails = [...formData.details];
        (newDetails[index] as any)[field] = value;
        setFormData(prev => ({ ...prev, details: newDetails }));
    };

    const handleMeasurementChange = (detailIndex: number, measurementKey: keyof SizeMeasurement, subField: 'min' | 'max', value: string) => {
        const newDetails = [...formData.details];
        const currentMeasurements = newDetails[detailIndex].measurements;
        // FIX: Add type assertion to ensure TypeScript knows this will be a tuple of two numbers, resolving the assignment error.
        const newRange: [number, number] = (currentMeasurements[measurementKey] || [0, 0]) as [number, number];
        
        newRange[subField === 'min' ? 0 : 1] = Number(value);

        newDetails[detailIndex].measurements = {
            ...currentMeasurements,
            [measurementKey]: newRange,
        };
        setFormData(prev => ({...prev, details: newDetails}));
    };
    ...
    const addSizeDetail = () => {
        setFormData(prev => ({
            ...prev,
            details: [...prev.details, { size: '', measurements: {} }],
        }));
    };
    ...
    return (
        <div className="modal open">
            <div className="modal-content modal-form-content" style={{ maxWidth: '800px', maxHeight: '90vh', overflowY: 'auto' }}>
                <button onClick={onClose} className="modal-close-btn">&times;</button>
                <h3 className="text-xl font-bold mb-4">{isEditing ? 'Edit Panduan Ukuran' : 'Tambah Panduan Ukuran'}</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="form-group"><label>Kategori</label><select name="category" value={formData.category} onChange={handleInputChange} className="form-select" required>{Object.values(Category).map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                        <div className="form-group">
                            <label>Sub Kategori</label>
                            <select name="style" value={formData.style || ''} onChange={handleInputChange} className="form-select" required disabled={subcategoryOptions.length === 0}>
                            ...
                            {subcategoryOptions.length === 0 && formData.category && (
                                <p className="text-xs text-gray-500 mt-1">Kategori ini tidak memiliki sub-kategori.</p>
                            )}
                        </div>
                    </div>
                    
                    <h4 className="text-lg font-semibold border-t pt-4">Detail Ukuran</h4>
                        ...
                        <table style={{ tableLayout: 'fixed' }}>
                            <thead>
                                <tr>
                                    ...
                                    {measurementFields.map(field => (
                                        <th key={field.key} colSpan={2}>{field.label} (cm)</th>
                                    ))}
                                ...
                                <tr>
                                    ...
                                    {measurementFields.map(field => (
                                        <React.Fragment key={field.key}>
                                            <th className="text-xs font-normal">Min</th>
                                            <th className="text-xs font-normal">Max</th>
                                        </React.Fragment>
                                    ))}
                                    <th></th>
                                </tr>
                            ...
                            <tbody>
                                {formData.details.map((detail, index) => (
                                    <tr key={index}>
                                        <td><input type="text" value={detail.size} onChange={(e) => handleDetailChange(index, 'size', e.target.value)} className="form-input text-sm p-1" required /></td>
                                        {measurementFields.map(field => (
                                            <React.Fragment key={field.key}>
                                                <td><input type="number" value={detail.measurements[field.key]?.[0] || ''} onChange={e => handleMeasurementChange(index, field.key, 'min', e.target.value)} className="form-input text-sm p-1" /></td>
                                                <td><input type="number" value={detail.measurements[field.key]?.[1] || ''} onChange={e => handleMeasurementChange(index, field.key, 'max', e.target.value)} className="form-input text-sm p-1" /></td>
                                            </React.Fragment>
                                        ))}
                                        <td>
                                            <button type="button" onClick={() => removeSizeDetail(index)} className="text-red-500"><i className="fas fa-trash-alt"></i></button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                    ...
                    <button type="button" onClick={addSizeDetail} className="form-button bg-gray-200 text-gray-800 hover:bg-gray-300 text-sm py-1 px-3">
                        + Tambah Baris Ukuran
                    </button>
...
export const DashboardModals: React.FC<DashboardModalsProps> = ({ modalState, onClose, showNotification }) => {
    ...
    const handleSaveSizeChart = (data: SizeChart) => {
        if (sizeCharts.some(c => c.id === data.id && c.id !== '')) { // Edit
            updateSizeCharts(prev => prev.map(c => c.id === data.id ? data : c));
            showNotification('Panduan ukuran berhasil diperbarui.');
        } else { // Add
            updateSizeCharts(prev => [...prev, { ...data, id: `sc${Date.now()}` }]);
            showNotification('Panduan ukuran baru berhasil ditambahkan.');
        }
        onClose();
    };

    const handleDeleteSizeChart = (id: string) => {
        updateSizeCharts(prev => prev.filter(c => c.id !== id));
        showNotification('Panduan ukuran berhasil dihapus.');
        onClose();
    };
    ...
    switch (modalState.type) {
        ...
        case 'add-size-chart':
            return <AddEditSizeChartModal onSave={handleSaveSizeChart} onClose={onClose} />;
        case 'edit-size-chart':
            return <AddEditSizeChartModal chart={modalState.chart} onSave={handleSaveSizeChart} onClose={onClose} />;
        case 'delete-size-chart':
            return <DeleteConfirmationModal title="Hapus Panduan Ukuran" message={`Anda yakin ingin menghapus panduan "${modalState.chart.name}"?`} onConfirm={() => handleDeleteSizeChart(modalState.chart.id)} onClose={onClose} />;
        default:
            return null;
    }
};
