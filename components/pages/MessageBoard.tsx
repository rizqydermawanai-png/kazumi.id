// components/pages/MessageBoard.tsx
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                <AnimatePresence>
                {messages.length > 0 ? messages.sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).map(msg => (
                    <motion.div 
                        key={msg.id} 
                        layout
                        ...
                        exit={{opacity: 0, x: -50}}
                        className="bg-white p-5 rounded-xl shadow-lg border relative"
                    >
                        {isAdmin && <Button variant="ghost" size="sm" onClick={() => handleDeleteMessage(msg.id)} className="absolute top-2 right-2 !p-2 h-auto text-slate-400 hover:bg-red-50 hover:text-red-500"><Trash2 size={16}/></Button>}
                        <div className="flex items-center mb-2">
                             <img src={users.find(u=>u.uid === msg.authorId)?.profilePictureUrl} alt="author" className="w-8 h-8 rounded-full mr-3"/>
                             <div>
