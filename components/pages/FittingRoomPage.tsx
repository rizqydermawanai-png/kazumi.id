...
const FittingRoomPage: React.FC = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!category || !subcategory || !chest || !waist) {
            showAlert('Harap isi pengukuran dada, pinggang, serta pilih kategori dan sub-kategori.');
            return;
        }
        ...
        const userMeasurements: Partial<Record<keyof SizeMeasurement, number>> = {
            chestWidth: parseInt(chest, 10),
            waist: parseInt(waist, 10),
            hip: parseInt(hip, 10),
            thigh: parseInt(thigh, 10),
        ...
        for (const sizeDetail of chart.details) {
            ...
            for (const key in sizeDetail.measurements) {
                ...
                if (userValue === undefined || isNaN(userValue)) {
                    showAlert(`Harap isi kolom pengukuran untuk ${measureKey}.`);
                    isMatch = false;
                    break; 
                }
